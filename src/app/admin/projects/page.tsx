    ...
    <div className="space-y-6">
      ...
      {deleteId != null && (
        <div className="rounded border border-red-200 bg-red-50 p-4">
          <p className="text-sm text-red-800">Bu proje kalıcı olarak silinecek. Emin misiniz?</p>
          <div className="mt-3 flex gap-2">
            ...
            <Link href="/admin/projects" className={adminGhostButtonClass}>
              Vazgeç
            </Link>
          </div>
        </div>
          ...
          <tbody>
            {projects.map((project) => (
              <tr key={project.id} className="border-b border-neutral-100">
                <td className="py-2 pr-3">{project.title}</td>
                <td className="py-2 pr-3 text-neutral-500">/{project.slug}</td>
                <td className="py-2 pr-3">{project.category}</td>
                <td className="py-2 pr-3">{project.results}</td>
                <td className="py-2 text-right">
                  <Link href={`/admin/projects/${project.id}`} className="text-neutral-700 underline underline-offset-2">
                    Düzenle
                  </Link>
                  <Link
